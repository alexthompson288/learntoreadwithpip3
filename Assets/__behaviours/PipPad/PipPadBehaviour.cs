...
public class PipPadBehaviour : Singleton<PipPadBehaviour>
{
	...
	public void Show(string word)
	{
		...
		string editedWord = word.Replace("!", "").Replace(".", "").Replace(",", "").Replace(";", "").ToLower().Replace('"', '?').Replace("?","")//.Replace("\"", "")
			.Replace("\'","").Replace("\n"," ");
		...
		try
		{
			...
			Debug.Log("There are " + dt.Rows.Count + " words of " + editedWord);
			...
			if (dt.Rows.Count > 0)
			{
				...
				if (areStarsActive || m_currentLanguage != "word")
				{
					...
					pbi.m_displayString = row[m_currentLanguage].ToString();
					pbi.m_positionIndex = 0;
					pbi.m_fullPhonemeId = -1;
					pbiList.Add(pbi);
					m_trickyStars.SetActive(areStarsActive);
